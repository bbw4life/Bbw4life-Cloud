// functions/fetch-cj-products.js — RÉCUPÉRATION DES VARIANTS CJ DROPSHIPPING (par lots)
// ⚠️ NOTE MIGRATION CLOUDFLARE : plusieurs sleep(1100ms) séquentiels par
// produit dans un lot (jusqu'à 3 produits) — cumul potentiellement proche
// des limites de durée d'exécution sur le plan Workers gratuit. Outil admin
// interne (pas un flux client), à surveiller après déploiement ; réduire
// BATCH_SIZE si besoin.

// ══════════════════════════════════════════════════════════════
//  Liste des Product IDs CJ à interroger.
// ══════════════════════════════════════════════════════════════
const PRODUCT_IDS = [
  { pid: "1798954511004999680" },
  { pid: "2510310626231633600" },
];
...
export async function onRequestGet(context) {
  ...
  const limit  = Math.max(1, parseInt(url.searchParams.get('limit'), 10) || BATCH_SIZE);
  ...
  if (!env.CJ_API_KEY) {
    log("  ❌  CJ_API_KEY manquant dans les variables d'environnement");
  ...
  if (!PRODUCT_IDS.length) {
    ...
    return new Response(JSON.stringify({ success: true, total: 0, offset, limit, hasMore: false, nextOffset, logs, products: [] }), {
  ...
  if (!batch.length) {
    ...
    return new Response(JSON.stringify({ success: true, total: PRODUCT_IDS.length, offset, limit, hasMore: false, nextOffset, logs, products: [] }), {
      status: 200, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
    });
  }
